...
const MaskedHeading: React.FC<MaskedHeadingProps> = ({
  ...
  // eslint-disable-next-line react-hooks/refs -- latest settings are only read inside the RAF drift loop below, never during render
        ...
        <span className="absolute inset-0 block" style={{ clipPath: `url(#${clipId})` }}>
          <span ref={mediaRef} className="absolute inset-0 block [will-change:transform,filter]">
            {mediaType === 'video' ? (
              ...
              // eslint-disable-next-line @next/next/no-img-element -- src is a runtime prop (image or remote URL), not a static import next/image can optimize
